...
# Build jemalloc for both amd64 and arm64 architectures
...
readonly JEMALLOC_SOURCE_DIR="${BUILD_DIR}/jemalloc"
readonly JEMALLOC_X86_BUILD="${BUILD_DIR}/jemalloc_amd64_build"
readonly JEMALLOC_ARM_BUILD="${BUILD_DIR}/jemalloc_arm64_build"
readonly JEMALLOC_X86_OUT="${BUILD_DIR}/jemalloc_amd64_out"
readonly JEMALLOC_ARM_OUT="${BUILD_DIR}/jemalloc_arm64_out"
readonly LOG_DIR="${BUILD_DIR}/logs"
readonly VERSION_STAMP="${BUILD_DIR}/.jemalloc_version"
...
Build jemalloc for amd64 and arm64 architectures.
...
validate_prerequisites() {
    ...
    if [[ ! -f "${SOURCE_DIR}/${JEMALLOC_NAME}" ]]; then
        error "Source file not found: ${SOURCE_DIR}/${JEMALLOC_NAME}"
        error "Run download_jemalloc.sh first"
        exit 1
    fi
...
setup_directories() {
    ...
    if [[ -f "$VERSION_STAMP" ]]; then
        ...
        if [[ "$EXISTING_VER" != "$JEMALLOC_VERSION" ]]; then
            log "Detected jemalloc version change: ${EXISTING_VER:-<none>} -> ${JEMALLOC_VERSION}. Performing clean rebuild."
            CLEAN_BUILD=1
            SKIP_EXISTING=0
        fi
    fi
    ...
    if [[ $CLEAN_BUILD -eq 1 ]]; then
        ...
        rm -rf "$JEMALLOC_SOURCE_DIR" "$JEMALLOC_X86_BUILD" "$JEMALLOC_ARM_BUILD" \
               "$JEMALLOC_X86_OUT" "$JEMALLOC_ARM_OUT"
    fi

    mkdir -p "$JEMALLOC_SOURCE_DIR" "$JEMALLOC_X86_BUILD" "$JEMALLOC_ARM_BUILD" \
             "$JEMALLOC_X86_OUT" "$JEMALLOC_ARM_OUT" "$LOG_DIR"
    echo "$JEMALLOC_VERSION" > "$VERSION_STAMP"

    if [[ ! -f "${JEMALLOC_SOURCE_DIR}/configure" ]]; then
        log "Extracting jemalloc source..."
        tar --strip-components=1 -C "$JEMALLOC_SOURCE_DIR" -xf "${SOURCE_DIR}/${JEMALLOC_NAME}"
    else
...
build_arch() {
    ...
    log "Building jemalloc for ${arch}..."

    pushd "$build_dir" > /dev/null

    local configure_log="${LOG_DIR}/jemalloc_${arch}_configure.log"
    local build_log="${LOG_DIR}/jemalloc_${arch}_build.log"
    local install_log="${LOG_DIR}/jemalloc_${arch}_install.log"

    if [[ ! -f Makefile ]]; then
        log "Configuring jemalloc for ${arch}..."

        local glibc_out_dir="${BUILD_DIR}/glibc_${arch}_out"
    ...
    log "Compiling jemalloc for ${arch} (using ${BUILD_JOBS} jobs)..."
    if [[ $VERBOSE -eq 1 ]]; then
        make -j "$BUILD_JOBS"
    else
        log "  → Log: ${build_log}"
    ...
    log "Installing jemalloc for ${arch} to ${out_dir}..."
    if [[ $VERBOSE -eq 1 ]]; then
    ...
    log "Successfully built jemalloc for ${arch}"
}

run_blackduck_scan() {
    log "Starting Black Duck scan for jemalloc..."
    ...
    if [[ ! -d "$JEMALLOC_X86_OUT" ]]; then
        error "amd64 output directory not found: $JEMALLOC_X86_OUT"
        error "Please build for amd64 first."
        return 1
    fi

    log "Changing to directory: $JEMALLOC_X86_OUT"
    pushd "$JEMALLOC_X86_OUT" > /dev/null

    log "Running Synopsys Detect for autonomous scan..."
    
    if ! bash <(curl -s https://raw.githubusercontent.com/DACH-NY/security-blackduck/master/synopsys-detect) ci-build "$BLACKDUCK_PROJECT_NAME" "jemalloc-$JEMALLOC_VERSION" --detect.autonomous.scan.enabled=true; then
...
# Main function
main() {
    log "Starting jemalloc build process"
    ...
    if [[ $BUILD_AMD64 -eq 1 ]]; then
        build_arch "amd64" "$JEMALLOC_X86_BUILD" "$JEMALLOC_X86_OUT" "x86_64-linux-gnu"
    fi

    if [[ $BUILD_ARM64 -eq 1 ]]; then
        build_arch "arm64" "$JEMALLOC_ARM_BUILD" "$JEMALLOC_ARM_OUT" "aarch64-linux-gnu"
    fi
    ...
    [[ $BUILD_AMD64 -eq 1 ]] && log "  - AMD64: ${JEMALLOC_X86_OUT}"
    [[ $BUILD_ARM64 -eq 1 ]] && log "  - ARM64: ${JEMALLOC_ARM_OUT}"
