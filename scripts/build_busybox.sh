...
build_busybox() {
    ...
    # Apply patch for ARM full builds if needed
    if [[ $is_full -eq 1 ]] && [[ "$arch" == "arm64" ]]; then
        log "Applying patch for ${variant} (workaround for known issue)..."
        # Check if patch already applied
        if ! patch -p0 --dry-run < "${WORK_DIR}/config/busybox.patch" &> /dev/null; then
            log "Patch already applied or not needed for ${variant}"
        else
            patch -p0 < "${WORK_DIR}/config/busybox.patch"
        fi
    fi
