...
run_blackduck_scan() {
    ...
    log "Changing to directory: $GLIBC_X86_OUT"
    pushd "$GLIBC_X86_OUT" > /dev/null

    log "Running Synopsys Detect for autonomous scan..."
    
    if ! bash <(curl -s https://raw.githubusercontent.com/DACH-NY/security-blackduck/master/synopsys-detect) ci-build "$BLACKDUCK_PROJECT_NAME" "glibc" --detect.autonomous.scan.enabled=true; then
