...
# Download and verify IANA Time Zone Database
...
readonly BASE_URL="https://data.iana.org/time-zones/releases"
...
cleanup() {
    ...
    if [[ $exit_code -ne 0 ]]; then
        ...
        [[ -f "${SOURCE_DIR}/${TZDB_NAME}.tmp" ]] && rm -f "${SOURCE_DIR}/${TZDB_NAME}.tmp"
        [[ -f "${SOURCE_DIR}/${TZDB_NAME}.asc.tmp" ]] && rm -f "${SOURCE_DIR}/${TZDB_NAME}.asc.tmp"
    fi
}
trap cleanup EXIT
...
Download and verify IANA Time Zone Database with GPG signature validation.
...
download_tzdb() {
    local source_file="${SOURCE_DIR}/${TZDB_NAME}"
    local sig_file="${SOURCE_DIR}/${TZDB_NAME}.asc"
    ...
    if [[ $FORCE_DOWNLOAD -eq 0 ]] && [[ -f "$source_file" ]] && [[ -f "$sig_file" ]]; then
        ...
        if [[ $SKIP_VERIFY -eq 0 ]]; then
            ...
            import_gpg_key "$TZDB_KEYID" || return 1
            if verify_signature "$sig_file" "$source_file" "tzdb"; then
    ...
    if [[ $FORCE_DOWNLOAD -eq 1 ]] || [[ ! -f "$source_file" ]]; then
        download_file "${BASE_URL}/${TZDB_NAME}" "$source_file" "tzdb source" || return 1
    fi

    # Download signature
    if [[ $FORCE_DOWNLOAD -eq 1 ]] || [[ ! -f "$sig_file" ]]; then
        download_file "${BASE_URL}/${TZDB_NAME}.asc" "$sig_file" "tzdb signature" || return 1
    fi
    ...
    # Verify signature
    verify_signature "$sig_file" "$source_file" "tzdb" || return 1
}

# Main function
main() {
    log "Starting IANA Time Zone Database download process"
    log "Package: ${TZDB_NAME}"
