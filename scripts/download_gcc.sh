...
# Download and verify GCC source for libstdc++
...
readonly BASE_URL="https://ftp.gnu.org/gnu/gcc/gcc-${GCC_VERSION}"
...
cleanup() {
    ...
    if [[ $exit_code -ne 0 ]]; then
        ...
        [[ -f "${SOURCE_DIR}/${GCC_NAME}.tmp" ]] && rm -f "${SOURCE_DIR}/${GCC_NAME}.tmp"
    fi
}
trap cleanup EXIT
...
Download and verify GCC source tarball.
...
download_gcc() {
    local package_file="${SOURCE_DIR}/${GCC_NAME}"
    local sig_file="${SOURCE_DIR}/${GCC_NAME}.sig"

    log "Processing GCC source..."
    ...
    if [[ $FORCE_DOWNLOAD -eq 0 ]] && [[ -f "$package_file" ]] && [[ -f "$sig_file" ]]; then
        ...
        if [[ $SKIP_VERIFY -eq 0 ]]; then
            import_gpg_key "$GCC_KEYID" || return 1
            if verify_signature "$sig_file" "$package_file" "GCC source"; then
    ...
    if [[ $FORCE_DOWNLOAD -eq 1 ]] || [[ ! -f "$package_file" ]]; then
        download_file "${BASE_URL}/${GCC_NAME}" "$package_file" "GCC source package" || return 1
    fi

    # Download signature
    if [[ $FORCE_DOWNLOAD -eq 1 ]] || [[ ! -f "$sig_file" ]]; then
        download_file "${BASE_URL}/${GCC_NAME}.sig" "$sig_file" "GCC source signature" || return 1
    fi
    ...
    # Verify signature
    import_gpg_key "$GCC_KEYID" || return 1
    verify_signature "$sig_file" "$package_file" "GCC source" || return 1
}

# Main function
main() {
    log "Starting GCC source download process"
    log "Version: ${GCC_VERSION}"
    ...
    log "  - GCC Source: ${SOURCE_DIR}/${GCC_NAME}"
}
