...
# Download and verify glibc source code
...
readonly BASE_URL="https://gnu.mirror.constant.com/glibc"
...
cleanup() {
    ...
    if [[ $exit_code -ne 0 ]]; then
        ...
        [[ -f "${SOURCE_DIR}/${GLIBC_NAME}.tmp" ]] && rm -f "${SOURCE_DIR}/${GLIBC_NAME}.tmp"
        [[ -f "${SOURCE_DIR}/${GLIBC_NAME}.sig.tmp" ]] && rm -f "${SOURCE_DIR}/${GLIBC_NAME}.sig.tmp"
    fi
}
trap cleanup EXIT
...
Download and verify glibc source code with GPG signature validation.
...
download_glibc() {
    local source_file="${SOURCE_DIR}/${GLIBC_NAME}"
    local sig_file="${SOURCE_DIR}/${GLIBC_NAME}.sig"
    ...
    if [[ $FORCE_DOWNLOAD -eq 0 ]] && [[ -f "$source_file" ]] && [[ -f "$sig_file" ]]; then
        ...
        if [[ $SKIP_VERIFY -eq 0 ]]; then
            ...
            import_gpg_key "$GLIBC_KEYID" || return 1
            if verify_signature "$sig_file" "$source_file" "glibc"; then
    ...
    if [[ $FORCE_DOWNLOAD -eq 1 ]] || [[ ! -f "$source_file" ]]; then
        download_file "${BASE_URL}/${GLIBC_NAME}" "$source_file" "glibc source" || return 1
    fi

    # Download signature
    if [[ $FORCE_DOWNLOAD -eq 1 ]] || [[ ! -f "$sig_file" ]]; then
        download_file "${BASE_URL}/${GLIBC_NAME}.sig" "$sig_file" "glibc signature" || return 1
    fi
    ...
    # Verify signature
    verify_signature "$sig_file" "$source_file" "glibc" || return 1
}

# Main function
main() {
    log "Starting glibc download process"
    log "Package: ${GLIBC_NAME}"
    ...
    log "File: ${SOURCE_DIR}/${GLIBC_NAME}"
}
