...
# Download and verify ncurses source code
...
readonly BASE_URL="https://gnu.mirror.constant.com/ncurses"
...
cleanup() {
    ...
    if [[ $exit_code -ne 0 ]]; then
        ...
        [[ -f "${SOURCE_DIR}/${NCURSES_NAME}.tmp" ]] && rm -f "${SOURCE_DIR}/${NCURSES_NAME}.tmp"
        [[ -f "${SOURCE_DIR}/${NCURSES_NAME}.sig.tmp" ]] && rm -f "${SOURCE_DIR}/${NCURSES_NAME}.sig.tmp"
    fi
}
trap cleanup EXIT
...
Download and verify ncurses source code with GPG signature validation.
...
download_ncurses() {
    local source_file="${SOURCE_DIR}/${NCURSES_NAME}"
    local sig_file="${SOURCE_DIR}/${NCURSES_NAME}.sig"
    ...
    if [[ $FORCE_DOWNLOAD -eq 0 ]] && [[ -f "$source_file" ]] && [[ -f "$sig_file" ]]; then
        ...
        if [[ $SKIP_VERIFY -eq 0 ]]; then
            ...
            import_gpg_key "$NCURSES_KEYID" || return 1
            if verify_signature "$sig_file" "$source_file" "ncurses"; then
    ...
    if [[ $FORCE_DOWNLOAD -eq 1 ]] || [[ ! -f "$source_file" ]]; then
        download_file "${BASE_URL}/${NCURSES_NAME}" "$source_file" "ncurses source" || return 1
    fi

    # Download signature
    if [[ $FORCE_DOWNLOAD -eq 1 ]] || [[ ! -f "$sig_file" ]]; then
        download_file "${BASE_URL}/${NCURSES_NAME}.sig" "$sig_file" "ncurses signature" || return 1
    fi
    ...
    # Verify signature
    verify_signature "$sig_file" "$source_file" "ncurses" || return 1
}

# Main function
main() {
    log "Starting ncurses download process"
    log "Package: ${NCURSES_NAME}"
    ...
    log "File: ${SOURCE_DIR}/${NCURSES_NAME}"
}
